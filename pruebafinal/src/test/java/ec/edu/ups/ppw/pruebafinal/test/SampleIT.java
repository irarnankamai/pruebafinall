package ec.edu.ups.ppw.pruebafinal.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/pruebafinal.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "pruebafinal.war").importFrom(f).as(WebArchive.class);

        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.ppw.pruebafinal.test");
